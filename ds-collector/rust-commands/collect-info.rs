...
fn execute_all_commands(options: &Options) {
    ...
    COMMANDS.iter().enumerate().for_each(|(_i, cmd)| {
        ...
        if !cmd.optional || check_command(cmd, &options) {
...
fn check_command(cmd: &Cmd, options: &Options) -> bool {
    let cmd_str = format_command(cmd.command, options);

    create_command("sh", &cmd, &options)
        .arg("-c")
...
fn format_args(args: &str, options: &Options, mask: bool) -> String {
    args.replace("{artifact_dir}", options.artifact_dir)
        ...
        .replace("{nodetool_credentials}", options.nodetool_credentials)
...
const COMMANDS: &[Cmd<'static>] = &[
    ...
    Cmd {
        command: "find",
        args: "{data_dir} -name *-Statistics.db -exec cp --parents {} {artifact_dir}/sstable-statistics/ ;",
        file: "",
        optional: true,
        skip_flags: "",
        use_stdout: true,
